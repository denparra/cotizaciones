...
# --- Sección fija en el sidebar para mensaje predeterminado de WhatsApp ---
with st.sidebar:
    st.markdown("### 📨 Mensaje predeterminado para WhatsApp")
    mensajes_raw = obtener_mensajes_whatsapp()
    if mensajes_raw:
        opciones = {f"{m[1]}": (m[2], m[3]) for m in mensajes_raw}
        mensaje_nombre = st.selectbox("Selecciona un mensaje:", list(opciones.keys()))
...
if opcion == "Inicio":
    ...
    for index, row in df_paginated.iterrows():
        row_key = f"row_{row['id']}"
        row_container = st.container(key=row_key)
        seleccionada = st.session_state.get('fila_activa') == row['id']
        if seleccionada:
            st.markdown(
                f"""
                <style>
                div[data-testid="stVerticalBlock"][data-key="{row_key}"] {{
...
# --- Mostrar el mensaje generado en el sidebar con botón para copiar ---
mensaje = st.session_state.get('mensaje_generado', 'No hay mensaje generado.')

# HTML personalizado para mostrar el mensaje y fijar el botón abajo
html_mensaje = f"""
...
# Renderiza el HTML en el sidebar
with st.sidebar:
