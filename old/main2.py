...
if opcion == "Inicio":
    ...
    with st.form("form_nueva"):
        ...
        if boton:
            if not telefono.strip():
                ...
                # Marcar para limpiar el formulario y reiniciar
    ...
    col_h1, col_h2, col_h3, col_h4, col_h5, col_h6, col_h7 = st.columns([2, 2, 2, 2, 2, 2, 1])
    col_h1.markdown("**Nombre**")
    col_h2.markdown("**Teléfono**")
    col_h3.markdown("**Auto**")
    col_h4.markdown("**WhatsApp**")
    col_h5.markdown("**Link Auto**")
    col_h6.markdown("**Copia**")
    col_h7.markdown("**ID**")

    # Iterar sobre cada fila y mostrar los datos junto con los botones
    for index, row in df_paginated.iterrows():
        r_col1, r_col2, r_col3, r_col4, r_col5, r_col6, r_col7 = st.columns([2, 2, 2, 2, 2, 2, 1])
        r_col1.write(row["nombre_cliente"])
        r_col2.write(row["telefono"])
        r_col3.write(row["auto"])
        ...
        r_col4.markdown(f"[Enviar]({whatsapp_url})")

        # Enlace para ver el link del auto (si existe)
        if row["link_auto"] and row["link_auto"].strip():
            r_col5.markdown(f"[Ver Auto]({row['link_auto']})")
        else:
            r_col5.write("—")

        # Botón "Copia" para cargar el mensaje en el área de texto
        if r_col6.button("Copia", key=f"copia_{row['id']}"):
            st.session_state['mensaje_generado'] = mensaje_personalizado
            st.success(f"Mensaje del contacto {row['nombre_cliente']} cargado para copiar.")
