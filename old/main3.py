...
import streamlit.components.v1 as components  # Para insertar HTML/JS
...
if opcion == "Inicio":
    ...
    # Encabezados de la tabla (10 columnas)
    col1, col2, col3, col4, col5, col6, col7, col8, col9, col10 = st.columns([3,3,3,3,3,3,3,3,3,1])
    col1.markdown("**Fecha**")
    ...
    for index, row in df_paginated.iterrows():
        r_col1, r_col2, r_col3, r_col4, r_col5, r_col6, r_col7, r_col8, r_col9, r_col10 = st.columns([3,3,3,3,3,3,3,3,3,1])
        ...
        # Botón "Copiar": carga el mensaje en el textarea
    ...
    # --- Botón HTML personalizado para copiar el mensaje ---
    # Se inyecta un componente HTML que renderiza un botón que, al hacer clic, ejecuta JavaScript para copiar el mensaje.
    copy_button_html = f"""
    <html>
      <head>
        <meta charset="utf-8">
      ...
      <body>
        <button onclick="copyText()" style="padding:10px 20px; font-size:16px;">Enviar y Copiar</button>
        <script>
          function copyText() {{
              var text = `{mensaje}`;
    ...
    """
    # Se asigna una altura suficiente para mostrar el botón (por ejemplo, 100px)
